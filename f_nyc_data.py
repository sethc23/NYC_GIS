...
class NYC_Data:
    ...
    def __init__(self,_parent_dict=None):
        ...
        self.T                              =   self.__load_libs__() if not _parent_dict else _parent_dict
        assert self.T.__class__.__name__=='To_Class'

        import requests
        from bs4                                import BeautifulSoup    as bsoup
        BASE_SND_PATH                       =   '/NYC_data/snd/'
        src_fpath                           =   BASE_SND_PATH + 'snd15Acow.txt'
        base_fpath                          =   src_fpath.rstrip('.txt')
        SND_NON_S_PATH                      =   base_fpath + '_non_s_recs.csv'
        SND_S_PATH                          =   base_fpath + '_s_recs.csv'

        for it in locals().keys():
            if not it=='self':
                self.T.update(                  { it                    :   eval(it) } )
    ...
    def create_numbered_streets(self,a='numbered_streets_from_lots',street_column='addr'):
    ...
    def get_latest_links_from_nyc_bytes(self):
        ...
        req                                 =   T.requests.get(nyc_bytes_url)
        html                                =   T.bsoup(req.content)
        data_sets                           =   html.find_all('table',attrs={'cellpadding':'5'})
        j,end                               =   '',False
        cols                                =   ['Data_Set','Date','link']
        df                                  =   T.pd.DataFrame(columns=cols)
        for _set in data_sets:
            ...
            for col in top_row.findAll('td'):
                if col.getText().strip(' \n'):
                    title                   =   T.re_sub(r'[^\x00-\x7F]+',' ', col.getText().strip(' \n')).encode(errors='ignore')
                    d.update(                   {'Data_Set'             :   title.replace('  ',' ')})
                    break
            ...
            for col in bot_row.findAll('td'):
                if col.getText():
                    ...
                    if txt.count('Date of Data')>0:
                        ...
                        d.update(               {'Date'                 :   txt.strip()})

                for l in col.findAll('a'):
                    if l.has_attr('href'):
                        if l.get('href').count('html'):
                            download_page   =   req.url[:req.url.rfind('/')+1] + l.get('href')
                            _req            =   T.requests.get(download_page)
                            _html           =   T.bsoup(_req.content)
                            _links=_html.find_all('a')
                            for it in _links:
                                if it.has_attr('href') and T.re_search('\.pdf|\.zip',it.get('href')):
                                    d.update(   {'link'                 :   os_path.normpath( _req.url[:_req.url.rfind('/')+1]
                                                                                + it.get('href') )})
        ...
        df['link']                          =   df.link.map(lambda s: s.strip() if s.find('http')==0
                                                                else os_path.normpath(os_path.join(BASE_URL,s.strip())))
        df['link']                          =   df.link.map(lambda s: s.replace('http:/www.','http://www.'))
        df['file_type']                     =   df.link.map(lambda s: s[s.rfind('.')+1:] if not s.count('?')
                                                                else s[s[:s.rfind('?')].rfind('.')+1:s.rfind('?')])
        # Clean Up
        idx                                 =   df[df.Data_Set.str.contains('^Copyright|^NYC Resources')].index.tolist()
        df                                  =   df.drop(idx,axis=0).reset_index(drop=True)
        return df

    def download_pad(self,df):
        """
    ...
    def show_other_datasets(self):
        more_NYC_data="""
        https://data.cityofnewyork.us/api/views/xx67-kt59/rows.xlsx?accessType=DOWNLOAD # DOHMH Results (08/28/15)
        http://data.cityofnewyork.us/resource/xx67-kt59.json # DOHMH Results (11/19/14)
        http://data.cityofnewyork.us/Health/Restaurant-Inspection-Results/4vkw-7nck
        http://data.cityofnewyork.us/resource/4vkw-7nck.json

        http://data.cityofnewyork.us/City-Government/Neighborhood-Names-GIS/99bc-9p23
        http://data.cityofnewyork.us/Recreation/Directory-Of-NYC-Cable-Providers/48pb-zy2g
        http://data.cityofnewyork.us/Recreation/Wifi-Hotspot-Locations/ehc4-fktp
        http://data.cityofnewyork.us/Social-Services/Zip-Codes-Map/zsjh-u7ve
        http://data.cityofnewyork.us/Education/Basic-Description-of-Colleges-and-Universities/42xp-mmeq
        http://data.cityofnewyork.us/Recreation/Directory-of-Dog-Runs-and-Off-Leash-Areas/ipbu-mtcs
        http://data.cityofnewyork.us/Housing-Development/Map-of-Parks/jc79-4imn
        http://data.cityofnewyork.us/Housing-Development/Map-of-Tracks/4wxj-39k8
        http://data.cityofnewyork.us/Recreation/Directory-of-Ice-Skating-Rinks/sage-tgxd
        http://data.cityofnewyork.us/Housing-Development/Directory-of-Bocce-Courts/qjqq-8zh3
        http://data.cityofnewyork.us/Transportation/Real-Time-Traffic-Speed-Data/xsat-x5sa
        http://data.cityofnewyork.us/Transportation/Alternate-Side-Parking/x6gf-qdui
        http://data.cityofnewyork.us/Transportation/Parking-Regulations-Street-Segments/9yzr-h7jq
        http://data.cityofnewyork.us/Transportation/Bicycle-Parking/qpbf-g2yx
        """
        print more_NYC_data

        NYC_neighborhoods = 'http://www.health.ny.gov/statistics/cancer/registry/appendix/neighborhoods.htm'
        zipcode_visual = 'http://www.citidex.com/map/zipco.html'
        print NYC_neighborhoods
        print zipcode_visual